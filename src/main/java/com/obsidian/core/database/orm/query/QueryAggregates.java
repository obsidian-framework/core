...
final class QueryAggregates
{
     ...
     * @param column   {@code *} for COUNT(*) or a validated column name
     * @return count, or {@code 0} if null
     */
     ...
     * @param executor JDBC executor
     * @param grammar  SQL grammar for WHERE compilation
     * @param function one of {@code COUNT}, {@code MAX}, {@code MIN}, {@code SUM}, {@code AVG}
     * @param column   target column, pre-validated by caller; {@code *} allowed for COUNT
     * @return aggregate value, or {@code null} if no rows matched
    ...
    static Object aggregate(QueryBuilder qb, QueryExecutor executor, Grammar grammar, String function, String column)
    {
        ...
        List<Map<String, Object>> rows = executor.executeQuery(sql.toString(), new ArrayList<>(qb.getBindings()));
